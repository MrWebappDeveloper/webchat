...
import '../../../../../../packages/chosen/chosen.min.css'
...
export default {
    ...
    data(){
        return{
            keyword:this.data.wizard.keyword,
            parent:this.data.wizard.parent_id,
            selectedFaqs:{},
    ...
    methods:{
        ...
        loadSelectedFaqs(){
            Request
                .get()
                .url('/wizard/' + this.data.wizard.id + '/edit')
                .success(function(response, instance){
                    let responseJson = JSON.parse(response)

                    instance.selectedFaqs = responseJson.faqs;
                })
                .use(this)
                .asyncSend();
        },
        ...
        onSubmit(e){
            ...
            $.ajax({
                type: "POST",
                url: "/wizard/" + this.data.wizard.id + "",
                data: formData,
                processData: false,
                contentType: false,
...
<template>
    <div class="padding-x-5 ">
        <h5>ویرایش ویزارد " {{ this.data.wizard.keyword }} "</h5>
        <form action="" @submit="onSubmit" v-if="!showSuccessResponse" class="padding-right-2 display-flex flex-dir-column">
            ...
            <div class="frm-group colu-6 padding-x-1">
                ...
                <select name="wizards" id="parent_wizard" class="multi-selection width-100 text-white" ref="wizardSelect" v-model="parent">
                    <option value="" disabled selected>ویزارد پدر را انتخاب کنید (اختیاری)</option>
                    <option v-for="wizard in wizards" :value="wizard.id" :selected="wizard.id === parent">{{ wizard.keyword }}</option>
                </select>
                <span class="invalid-message"></span>
            </div>
            <div class="frm-group colu-6 padding-x-1">
                ...
                <select v-model="selectedFaqs" name="faqs" id="" class="multi-selection width-100 text-white" data-placeholder="پاسخ های ویزارد جدید را انتخاب کنید ..." ref="faqsSelect" multiple>
                    <option v-for="faq in faqs" :value="faq.id" :selected="selectedFaqs.includes(faq.id)">{{ faq.question }}</option>
                </select>
                <span class="invalid-message"></span>
            </div>
            <div class="frm-group colu-12 display-flex content-end">
                <input type="submit" value="ویرایش" class="button button-primary">
        ...
        <div class="success-response width-100 display-flex content-center" v-if="showSuccessResponse">
            <div class="box-shadow text-align-center border-radius padding-5 colu-4">
                ...
                <p class="text-align-center padding-top-5">ویزارد با موفقیت ویرایش شد !</p>
